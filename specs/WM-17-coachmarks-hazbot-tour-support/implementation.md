...
**Decision (updated during implementation): A — export the designer's coach-mark variants from Zeplin.** The original plan chose B (composite wildfire's bottom-bar `hazbot-back.svg` + `hazbot-eyes.svg`), and that shipped first. Visual review against the Zeplin design (and the design owner) showed the design has **dedicated coach-mark variants** with a more rounded bottom — distinct from the bottom-bar **Button** art — so the asset was re-exported from the **"Hazbot Coach Mark - Back"** + **"Hazbot Coach Mark - Eyes"** (open eyes, not Blinks) layers (project `5fe47ae231d1f6a428c53450`, screen `69b2baa489a2e2f3308238b8`) and composited into a single 52×52 `src/styles/assets/hazbot-avatar.svg`. This is the spec's fallback-to-A path, taken because design wanted the badge-specific variant.

**Provenance / drift guard:** the committed SVG is a *point-in-time composite* of the Zeplin **Hazbot Coach Mark - Back** + **Hazbot Coach Mark - Eyes** layers — the canonical face lives in the design (and in wildfire's in-app robot) with no build-time link. The leading comment inside the committed `hazbot-avatar.svg` records the exact layer source IDs / project / screen and notes it must be re-exported if the design changes. A design/robot restyle is therefore a known cross-repo coupling (see requirements.md Out of Scope), not a silent divergence.
